...
class App(object):
    ...
    def __init__(self, root):
        ...
        ww = 850
        wh = 20
        self.root.geometry("%dx%d+%d+%d" % (ww, wh, (sw - ww) / 2, (sh - wh) / 2 * 1.92))
        ...
        Button(self.frame, text=self.cho[0], command=self.c0, width=18, height=1,

                    activebackground='grey', relief='groove').grid(row=0, column=0)
        Button(self.frame, text=self.cho[1], command=self.c1, width=18, height=1,

                    activebackground='grey', relief='groove').grid(row=0, column=3)
        Button(self.frame, text=self.cho[2], command=self.c2, width=18, height=1,

                    activebackground='grey', relief='groove').grid(row=0, column=6)
        Button(self.frame, text=self.cho[3], command=self.c3, width=18, height=1,

                    activebackground='grey', relief='groove').grid(row=0, column=9)
        Button(self.frame, text=self.cho[4], command=self.c4, width=18, height=1,

                    activebackground='grey', relief='groove').grid(row=0, column=12)
        Button(self.frame, text=self.cho[5], command=self.c5, width=18, height=1,

                    activebackground='grey', relief='groove').grid(row=0, column=15)
        Button(self.frame, text=self.cho[6], command=self.c6, width=9, height=1,

                    activebackground='grey', relief='groove').grid(row=0, column=18)
        Button(self.frame, text=self.cho[7], command=self.c7, width=9, height=1,

                    activebackground='grey', relief='groove').grid(row=0, column=21)
...
class Pcode(object):
    ...
    def __init__(self, root):
        ...
        ww = 400
        wh = 70
        self.root.geometry("%dx%d+%d+%d" % (ww, wh, (sw - ww) / 2, int((sh - wh) / 2 )*0.8))
        self.root.after(1, lambda:self.root.focus_force())
        self.inf = StringVar()
        self.l = Label(self.root, textvariable=self.inf, font=('微软雅黑', 10))
        self.inf.set('Last consultion takes: '+str(ttos(Tv.dur_s))+'\nNo:'+str(Tv.p_count+1)+'('+str(Tv.speed)+'/per patient)')
    
        self.l.pack()
        self.frame = Frame(self.root)
        self.frame.pack()
        self.e = Entry(self.frame, relief='groove',width=40,font=('微软雅黑', 10),bg='silver')
        self.e.focus_set()
        ...
        Button(self.frame, text='取消', command=self.c1, width=10, height=1,

                    activebackground='grey', relief='groove',font=('黑体', 10)).pack(side=RIGHT)
...
class Drug(object):  
    drug = {'klsfs':'头孢克肟颗粒', 'zdsk':'克洛己新干混悬剂','afh':'小儿氨酚黄那敏颗粒', 'bxp':'头孢丙烯片', 'kwk':'奥司他韦颗粒',
            'akn':'阿克诺片', 'lhjn':'罗红霉素胶囊', 'jbkfy':'解表口服液', 'dckfy':'定喘口服液', 'qly':'鱼腥草芩蓝合剂',
            'jqkl':'小儿金翘颗粒', 'jby':'桔贝合剂', 'pkkm':'克咳敏片', 'psgd':'赛庚啶片', 'pacj':'氨茶碱片', 'ylha':'氯化铵',
            'klmlst':'孟鲁司特颗粒', 'wcz':'维生素C',
            'rdz':'热毒宁', 'mnz':'头孢米诺(福安)', 'jsz':'头孢米诺(金石)', 'xdz':'头孢西丁', 'gxz':'更昔洛韦',
            'dxz':'地塞米松', 'tdz':'头孢他啶', 'ndz':'头孢曲松(那单)', 'yhz':'炎琥宁',
            'tqz':'头孢曲松(立健松)', 'aqz':'小阿奇', 'qxz':'大阿奇', 'lyz':'拉氧头孢0.5',
            'akz':'阿莫西林克拉维酸钾针', 'xmn':'头孢米诺(欣峰)'}
            
    drugs = {'mnrd':('mnz','rdz'), 'tqrd':('tqz','rdz'), 'tdrd': ('tdz','rdz'), 'lygx':('lyz','gxz'),'tqyh':('tqz', 'yhz'),
             'xdrd':('xdz', 'rdz'), 'mnyh':('mnz','yhz'),
             'akrd':('akz','rdz'),'fr':('jqkl','afh','kwk','bxp'),
             'ks':('pkkm','psgd','ylha','bxp','jqkl'), 'zdkw':('zdsk','kwk','jqkl','afh'),
              'tqgx':('tqz','gxz')}        
...
while True:
    ...
    if not bingren:
        ...
        pyautogui.click(loop_loc('.\\shot\就诊卡.png'))
    ...
    while True:
        ...
        ww1 = 200
        wh1 = 670
        root1.geometry("%dx%d+%d+%d" % (ww1, wh1, (sw1 - ww1) / 2, int((sh1 - wh1) / 2)*1.4))
        ...
        zd.set(('呼吸道感染','化脓性扁桃体炎','疱疹性咽峡炎','口炎','粒细胞减少症','SIRS','急性喉炎','支气管炎',
        '肺炎','毛细支气管炎','喘息性支气管炎','胃肠炎','肠炎','胃肠功能紊乱','腹痛','呕吐',
        '肠系膜淋巴结炎','急性荨麻疹','皮疹','颅内感染','头痛','头晕','植物神经功能紊乱','泌尿系感染','过敏性紫癜','疫苗接种反应',
        '周围神经炎','癫痫','惊厥','药物中毒','农药中毒'))
        lb1 = Listbox(fz, listvariable=zd, selectmode=MULTIPLE,bd=1,relief='groove',font=('微软雅黑', 10), selectbackground='brown',
                     width=25, height=32, yscrollcommand=crollz.set)
        lb1.focus_set()
    ...
    while True:
        ...
        if m == '血常规':
            ...
            ww2 = 600
            wh2 = 700
            rot.geometry("%dx%d+%d+%d" % (ww2, wh2, (sw2 - ww2) / 2, (sh2 - wh2) / 2))
            ...
            ww21 = 600
            wh21 = 700
            rot1.geometry("%dx%d+%d+%d" % (ww21, wh21, (sw21 - ww21) / 2, (sh21 - wh21) / 2))
            ...
            class Rp(object):
                ...
                def __init__(self, root):
                    ...
                    self.e1 = Entry(self.root, textvariable=self.t_age,width=10,font=('微软雅黑', 10))
                    self.e1.focus_set()
                    self.e1.pack()
                    ...
                    self.e2 = Entry(self.root, textvariable=self.t_weight,width=10,font=('微软雅黑', 10))
                    self.e2.pack()
                    
                    l3 = Label(self.root, textvariable=self.t_input_show)
                    l3.pack()
        
                    self.e3 = Entry(self.root, textvariable=self.t_input, width=30,font=('微软雅黑', 12))
                    ...
                    self.lb = Listbox(f, listvariable=self.val, selectmode=MULTIPLE, bd=1,font=('微软雅黑', 10), selectbackground='brown',
                                 width=50, height=25, yscrollcommand=croll.set)
                    ...
                    btc = Button(fdrug, text='取 消', command=self.bcc, width=12, height=2,
            ...
            if rp.code_input:
                ...
                pyautogui.alert(text='患儿年龄：{:0>2d} 岁\n患儿体重：{:0>2d} kg'.format(int(para.age_s), int(para.weight_s)), title='请确认:！', button='OK')
